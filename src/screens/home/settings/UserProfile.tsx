...
const UserProfile = () => {
  ...
  const openCamera = async () => {
    try {
      ...
      Alert.alert('Error', `Image picker is not available: ${error.message || 'Unknown error'}`);
    }
  };
  ...
  const openImageLibrary = async () => {
    try {
      ...
      (ImagePicker as any).launchImageLibrary(options as any, (response: any) => {
        if (response.didCancel) {
      ...
      console.error('Failed to load image picker or launch image library:', error);
      Alert.alert('Error', `Image picker is not available: ${error.message || 'Unknown error'}`);
    }
  };
